...
# rembg 세션 초기화 (더 높은 품질의 모델 사용)
rembg_session = new_session('u2net')  # u2net, u2netp, u2net_human_seg 중 선택 가능
...
def analyze_image_with_gemini_for_bg_removal(image_data: bytes, model_name: str = "gemini-1.5-flash") -> dict:
    """
...
def remove_background_advanced(image_data: bytes, analysis: dict) -> bytes:
    """
    고급 배경 제거 처리 (Gemini 분석 결과 활용)
    ...
    """
    try:
        print("🎨 고급 배경 제거 처리 시작")
        
        # 분석 결과에 따라 적절한 모델 선택
        complexity = analysis.get('complexity', 'medium')
        has_person = analysis.get('has_person', False)
        
        # 사람이 있는 경우 human_seg 모델 사용
        if has_person:
            session = new_session('u2net_human_seg')
            print("👤 사람 감지됨 - u2net_human_seg 모델 사용")
        elif complexity == 'hard':
            session = new_session('u2netp')  # 더 정밀한 모델
            print("🔧 복잡한 배경 - u2netp 모델 사용")
        else:
            session = rembg_session  # 기본 u2net 모델
            print("✨ 일반 배경 - u2net 모델 사용")
        
        # 배경 제거 수행
        output_data = remove(
            image_data,
            session=session,
            alpha_matting=True,  # 알파 매팅 활성화로 더 부드러운 경계
        ...
        # 경계 부드럽게 처리
        alpha_smooth = gaussian_filter(alpha.astype(float), sigma=0.5)
...
def remove_background_from_url(image_url: str) -> Optional[bytes]:
    ...
    try:
        ...
        # Gemini로 이미지 분석 (GEMINI_API_KEY가 있는 경우에만)
        analysis = {}
        if os.getenv("GEMINI_API_KEY"):
            print("🤖 Gemini를 사용한 이미지 분석 시작...")
            analysis = analyze_image_with_gemini_for_bg_removal(image_data)
            
            # 분석 결과가 있고 에러가 없으면 고급 배경 제거 사용
            if analysis and not analysis.get('error'):
                print("🎨 Gemini 분석 기반 고급 배경 제거 중...")
                background_removed_data = remove_background_advanced(image_data, analysis)
            else:
                print("🎭 기본 배경 제거 중 (Gemini 분석 실패)...")
                background_removed_data = remove(image_data, session=rembg_session)
        else:
            print("🎭 기본 배경 제거 중 (Gemini API 키 없음)...")
            background_removed_data = remove(image_data, session=rembg_session)
